...
from desktop.desktop_items import *
import settings
...
def information():
    return render_template("folders/information.html")


@app.route('/tutorial/')
def tutorial():
    return render_template("programs/tutorial.html")


@app.route('/cmd/')
def cmd():
    cmd = request.args.get("cmd", "os2wnd")
    return render_template("programs/cmd_%s.html" % cmd)


@app.route('/webexplorer/')
def webexplorer():
    return render_template("programs/webexplorer.html")


@app.route('/ajax_query/<query_id>/')
def ajax_query(query_id):
    return eval(query_id + '_data()')


@app.route('/system_folder/')
def system_folder():
    items = []
    ...
    return render_template("folders/system_folder.html", **content)
...
"""
    ...
    return render_template("programs/%s.html" % app_name, **content)
